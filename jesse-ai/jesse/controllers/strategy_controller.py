...
async def index_jesse_trade_strategies(
    ...
    """
    Browse strategies from localhost
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()
    ...
    try:
        ...
        return JSONResponse({
            'status': 'error',
            'message': f'Error connecting to localhost: {str(e)}'
        },         status_code=500)
...
async def get_jesse_trade_periods(
    ...
    """
    Get available trading periods from localhost
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()
...
async def get_jesse_trade_strategy(
    ...
    """
    Get a specific strategy from localhost
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()
...
async def get_jesse_trade_strategy_metrics(
    ...
    """
    Get metrics for a specific strategy from localhost
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()
...
async def import_strategy(
    ...
    """
    Import a strategy from localhost
    """
    if not authenticator.is_valid_token(authorization):
        return authenticator.unauthorized_response()

    try:
        # Fetch the strategy from localhost
        headers = {}
        if jesse_trade_token:
            headers['Authorization'] = f'Bearer {jesse_trade_token}'
