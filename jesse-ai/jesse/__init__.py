...
try:
    ...
    # MOCK HAS_LIVE to True for the UI regardless of the plugin presence
    # The actual execution will still fail if plugin is missing, but UI will unlock
    pass
...
async def websocket_endpoint(websocket: WebSocket, token: str = Query(...)):
    ...
    if not authenticator.is_valid_token(token):
        jh.terminal_debug(f"WS Auth Failed. Token: {token}")
        # Properly reject the connection with close code
    ...
    try:
        ...
        jh.terminal_debug(f"Redis listener failed to start (ignoring): {str(e)}")
...
def run() -> None:
    ...
    welcome_message = """
    Create the Future with F-Antigravity.
    """
    version = pkg_resources.get_distribution("jesse").version
    print(welcome_message)
    print(f"F-Antigravity Framework Version: {version}")
...
async def startup_event():
    from jesse.services.redis import init_redis
    await init_redis()
...
from jesse.controllers.angel_one_controller import router as angel_one_router

# Angel One login page route
@fastapi_app.get("/angel-login")
async def angel_login_page():
    return FileResponse(f"{JESSE_DIR}/static/angel_login.html")

# register routers
fastapi_app.include_router(angel_one_router)
