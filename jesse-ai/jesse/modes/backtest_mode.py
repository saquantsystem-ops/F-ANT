...
def load_candles(start_date: int, finish_date: int) -> Tuple[dict, dict]:
    ...
    for c in config['app']['considering_candles']:
        ...
        # Patch for YFinance: If warmup is missing, steal from trading candles
        if exchange == 'YFinance' and (warmup_candles_arr is None or len(warmup_candles_arr) == 0):
            # Since YFinance candles are expanded to 1m, warmup_num needs to be multiplied by 1440
            warmup_minutes = warmup_num * 1440
            if trading_candle_arr is not None and len(trading_candle_arr) > warmup_minutes:
                print(f"YFinance Patch: Taking {warmup_num} days ({warmup_minutes} minutes) warmup candles from trading set.")
                warmup_candles_arr = trading_candle_arr[:warmup_minutes]
                trading_candle_arr = trading_candle_arr[warmup_minutes:]

        # BYPASSED: Skip strict timestamp validation since we're using yfinance data
        # The candle timestamps may not exactly match the requested dates
