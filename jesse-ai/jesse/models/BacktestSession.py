...
def store_backtest_session(
    ...
    session = get_backtest_session_by_id(id)
    
    if session:
        session.status = status
        session.updated_at = jh.now_to_timestamp(True)
        session.save()
    else:
        d = {
            'id': id,
...
def update_backtest_session_status(id: str, status: str) -> None:
    try:
        BacktestSession.update(
            status=status,
            updated_at=jh.now_to_timestamp(True)
        ).where(BacktestSession.id == id).execute()
    except Exception:
        pass
    ...
    try:
        ...
        if days_old is not None and days_old > 0:
            ...
            for session_id in sessions_to_delete:
                try:
                    if delete_backtest_session(str(session_id)):
                        deleted_count += 1
                except Exception:
                    pass
        else:
            ...
            for session in all_sessions:
                try:
                    if delete_backtest_session(str(session.id)):
                        deleted_count += 1
                except Exception:
                    pass
